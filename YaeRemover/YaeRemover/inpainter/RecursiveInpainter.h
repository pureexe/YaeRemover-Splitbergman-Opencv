...
Mat RecusriveInpainter(Mat original_image, Mat inpainted_domain, double lambda, double theta, double tolerant, int iteration_coarse, int iteration_mid, int iteration_fine, int max_step, int current_step)
{
	std::wstring str;
	Mat toinpaint_image;
	...
	if (current_step < max_step) {
		...
		half_result = RecusriveInpainter(half_image, half_domain, lambda, theta, tolerant, iteration_coarse, iteration_mid, iteration_fine, max_step, current_step + 1);
